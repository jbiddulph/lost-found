...
//  LostFound
//
//  Created by John Biddulph on 25/06/2017.
...
class ChatLogController: UICollectionViewController, UITextFieldDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func handleSend() { 
        ...
        let filteredBy = self.itemId+"_"+self.theeventId
        let timeStamp = Int(NSDate().timeIntervalSince1970)
        let values = ["text":inputTextField.text!, "toId": toId, "fromId": fromId, "timestamp": timeStamp, "theFilter": filteredBy] as [String : Any]
        ...
        childRef.updateChildValues(values) { (error, ref) in
            ...
            let recipientUserMessagesRef = Database.database().reference().child("item-messages").child("user-messages").child(toId)
